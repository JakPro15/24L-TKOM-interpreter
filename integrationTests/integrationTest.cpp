...
{
    checkOutputFromArgs("factorialTest/factorialUse.txt --args 3", "6\n", "", Succeeded);
    checkOutputFromArgs("factorialTest/factorial.txt --args 4", "24\n", "", Succeeded);
    checkOutputFromArgs("factorialTest/factorialUse.txt --args 10", "3628800\n", "", Succeeded);
    checkOutputFromArgs("factorialTest/factorial.txt", "no argument given\n", "", Succeeded);
    checkOutputFromArgs(
        "factorialTest/factorialUse.txt --args -1", "",
        "The program was terminated following a runtime error:\n"
        "Recursion limit exceeded\n"
        "while executing file factorialTest/factorial.txt\n"
        "at line 6, column 20.\n",
        Failed
    );
    checkOutputFromArgs(
        "factorialTest/factorial.txt factorialTest/factorialUse.txt --args abc", "",
        "The program was terminated following a runtime error:\n"
        "Conversion of string abc to integer failed\n"
        "while executing file factorialTest/factorialUse.txt\n"
        "at line 8, column 23.\n",
        Failed
    );
}
...
{
    checkOutputFromArgs(
        "errorTests/syntaxError.txt", "",
        "Error: Expected '(', got '}'\n"
        "in file errorTests/syntaxError.txt\n"
        "at line 2, column 1.\n",
        Failed
    );
    checkOutputFromArgs(
        "errorTests/includedError.txt", "",
        "Error: Expected '(', got '}'\n"
        "in file errorTests/syntaxError.txt\n"
        "at line 2, column 1.\n",
        Failed
    ...
    checkOutputFromArgs(
        "errorTests/collision1.txt", "",
        "Error: Duplicate function with signature input\n"
        "in file errorTests/collision1.txt\n"
        "at line 1, column 1.\n",
        Failed
    );
    checkOutputFromArgs(
        "errorTests/collision2.txt", "",
        "Error: Duplicate function with signature main\n"
        "in file factorialTest/factorialUse.txt\n"
        "at line 3, column 1.\n",
        Failed
    );
}
...
{
    checkOutputFromArgs(
        "stdinTest/lineNumberAdder.txt", "1 first line\n2 second line\n3 third line\n", "", Succeeded,
        "stdinTest/input.txt"
    );
    checkOutputFromArgs(
        "stdinTest/lineNumberAdder.txt --args a", "a1 first line\na2 second line\na3 third line\n", "", Succeeded,
        "stdinTest/input.txt"
    );
}

TEST_CASE("structs and variants program", "[IntegrationTests]")
{
    checkOutputFromArgs(
        "typesTest/main.txt", "An integer: 23\nA string: 'heeello!'\nAn integer: 22\nA string: 'not hello >:('\n", "",
        Succeeded
    );
}
...
{
    std::string
        lineNumberAdderDt = "Program containing:\n"
                            "Functions:\n"
                            "`-main: FunctionDeclaration <line: 1, col: 1> source=stdinTest/lineNumberAdder.txt\n"
                            " `-Body:\n"
    ...
    checkOutputFromArgs("stdinTest/lineNumberAdder.txt --dump-dt", lineNumberAdderDt, "", Succeeded);
}
